...
import (
	...
	"github.com/showtime/server/config/ipdns"
	"github.com/showtime/server/config/mode"
)

var (
	prefix        = "showtime"
	files         = []string{"showtime.config.development.local", "showtime.config.development", "showtime.config.local", "showtime.config"}
	absoluteFiles = []string{"/etc/showtime/server.config"}
...
func Get() (Config, []FutureLog) {
	...
	if config.AuthMode != AuthModeTurn && config.AuthMode != AuthModeAll && config.AuthMode != AuthModeNone {
		logs = append(logs,
			futureFatal(fmt.Sprintf("invalid SHOWTIME_AUTH_MODE: %s", config.AuthMode)))
	}

	if config.ServerTLS {
		if config.TLSCertFile == "" {
			logs = append(logs, futureFatal("SHOWTIME_TLS_CERT_FILE must be set if TLS is enabled"))
		}

		if config.TLSKeyFile == "" {
			logs = append(logs, futureFatal("SHOWTIME_TLS_KEY_FILE must be set if TLS is enabled"))
		}
	}
	...
	if len(config.Secret) == 0 {
		...
		if _, err := rand.Read(config.Secret); err == nil {
			logs = append(logs, FutureLog{
				...
				Msg:   "SHOWTIME_SECRET unset, user logins will be invalidated on restart",
			})
		} else {
	...
	if len(config.TurnExternalIP) > 0 {
		if len(config.ExternalIP) > 0 {
			logs = append(logs, futureFatal("SHOWTIME_EXTERNAL_IP and SHOWTIME_TURN_EXTERNAL_IP must not be both set"))
		}

		config.TurnIPProvider, errs = parseIPProvider(config.TurnExternalIP, "SHOWTIME_TURN_EXTERNAL_IP")
		config.TurnPort = config.TurnExternalPort
		config.TurnExternal = true
		logs = append(logs, errs...)
		if config.TurnExternalSecret == "" {
			logs = append(logs, futureFatal("SHOWTIME_TURN_EXTERNAL_SECRET must be set if external TURN server is used"))
		}
	} else if len(config.ExternalIP) > 0 {
		config.TurnIPProvider, errs = parseIPProvider(config.ExternalIP, "SHOWTIME_EXTERNAL_IP")
		logs = append(logs, errs...)
		split := strings.Split(config.TurnAddress, ":")
		config.TurnPort = split[len(split)-1]
	} else {
		logs = append(logs, futureFatal("SHOWTIME_EXTERNAL_IP or SHOWTIME_TURN_EXTERNAL_IP must be set"))
	}

	min, max, err := config.parsePortRange()
	if err != nil {
		logs = append(logs, futureFatal(fmt.Sprintf("invalid SHOWTIME_TURN_PORT_RANGE: %s", err)))
	} else if min == 0 && max == 0 {
		// valid; no port range
	} else if min == 0 || max == 0 {
		logs = append(logs, futureFatal("invalid SHOWTIME_TURN_PORT_RANGE: min or max port is 0"))
	} else if min > max {
		logs = append(logs, futureFatal(fmt.Sprintf("invalid SHOWTIME_TURN_PORT_RANGE: min port (%d) is higher than max port (%d)", min, max)))
	} else if (max - min) < 40 {
		logs = append(logs, FutureLog{
			Level: zerolog.WarnLevel,
	...
	for _, cidrString := range config.TurnDenyPeers {
		...
		if err != nil {
			logs = append(logs, FutureLog{
				...
				Msg:   fmt.Sprintf("Invalid SHOWTIME_TURN_DENY_PEERS %q: %s", cidrString, err),
			})
		} else {
...
func logDeprecated() []FutureLog {
	if os.Getenv("SHOWTIME_TURN_STRICT_AUTH") != "" {
		return []FutureLog{{Level: zerolog.WarnLevel, Msg: "The setting SHOWTIME_TURN_STRICT_AUTH has been removed."}}
	}
	return nil
}
...
func getFiles(relativeTo string) []string {
	...
	if err == nil {
		result = append(result, filepath.Join(homeDir, ".config/showtime/server.config"))
	}
